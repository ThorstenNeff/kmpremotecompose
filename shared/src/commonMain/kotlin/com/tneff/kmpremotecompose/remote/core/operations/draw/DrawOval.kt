...
/**
 * `DRAW_OVAL` (opcode [Operations.DRAW_OVAL]) — draw an oval inscribed in a rectangle.
 *
 * Wire layout: opcode byte followed by four big-endian IEEE-754 floats `left, top, right, bottom`
 * (mirrors upstream `DrawOval` / `DrawBase4`). Floats may carry NaN-encoded ids; raw bits preserved.
 */
class DrawOval(
    val left: Float,
    val top: Float,
    ...
    override val opcode: Int get() = Operations.DRAW_OVAL
    ...
    override fun paint(context: RemoteContext, paint: PaintContext) {
        paint.drawOval(rLeft, rTop, rRight, rBottom)
    }

    override fun dump(): String = "DRAW_OVAL l=$left t=$top r=$right b=$bottom"
    ...
    companion object : OperationReader {
        override fun read(buffer: WireBuffer, operations: MutableList<Operation>) {
            operations += DrawOval(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat())
        }
    }
}
