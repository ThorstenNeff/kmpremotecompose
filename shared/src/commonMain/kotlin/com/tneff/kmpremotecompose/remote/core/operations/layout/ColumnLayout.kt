...
/**
 * `LAYOUT_COLUMN` (opcode [Operations.LAYOUT_COLUMN]) — a column layout container.
 *
 * Wire layout: opcode byte + int `componentId` + int `animationId` + int `horizontalPositioning` +
 * int `verticalPositioning` + float `spacedBy` = 21 bytes (mirrors upstream `ColumnLayout.apply`/`read`;
 * like [BoxLayout] plus the `spacedBy` gap). `spacedBy` may carry a NaN-encoded id; raw bits preserved.
 */
class ColumnLayout(
    val componentId: Int,
    val animationId: Int,
    ...
    override val opcode: Int get() = Operations.LAYOUT_COLUMN
    ...
    override fun dump(): String =
        "LAYOUT_COLUMN id=$componentId anim=$animationId hPos=$horizontalPositioning " +
