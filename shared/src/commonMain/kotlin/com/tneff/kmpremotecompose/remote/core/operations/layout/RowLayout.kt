...
/**
 * `LAYOUT_ROW` (opcode [Operations.LAYOUT_ROW]) — a row layout container.
 *
 * Wire layout: opcode byte + int `componentId` + int `animationId` + int `horizontalPositioning` +
 * int `verticalPositioning` + float `spacedBy` = 21 bytes (mirrors upstream `RowLayout.apply`/`read`;
 * same shape as [ColumnLayout]). `spacedBy` may carry a NaN-encoded id; raw bits preserved.
 */
class RowLayout(
    val componentId: Int,
    val animationId: Int,
    ...
    override val opcode: Int get() = Operations.LAYOUT_ROW
    ...
    override fun dump(): String =
        "LAYOUT_ROW id=$componentId anim=$animationId hPos=$horizontalPositioning " +
