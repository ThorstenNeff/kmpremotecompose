...
/**
 * `LAYOUT_COLLAPSIBLE_COLUMN` (opcode [Operations.LAYOUT_COLLAPSIBLE_COLUMN]) — a collapsible column
 * layout container.
 *
 * Wire layout: opcode byte + int `componentId` + int `animationId` + int `horizontalPositioning` +
 * int `verticalPositioning` + float `spacedBy` (mirrors upstream `CollapsibleColumnLayout.apply`/`read`).
 * `spacedBy` may carry a NaN-encoded id; raw bits preserved.
 */
class CollapsibleColumnLayout(
    val componentId: Int,
    val animationId: Int,
    ...
    override val opcode: Int get() = Operations.LAYOUT_COLLAPSIBLE_COLUMN
    ...
    override fun dump(): String =
        "LAYOUT_COLLAPSIBLE_COLUMN id=$componentId anim=$animationId hPos=$horizontalPositioning " +
            "vPos=$verticalPositioning spacedBy=$spacedBy"

    override fun equals(other: Any?): Boolean =
        this === other || (
            other is CollapsibleColumnLayout &&
    ...
    companion object : OperationReader {
        override fun read(buffer: WireBuffer, operations: MutableList<Operation>) {
            operations += CollapsibleColumnLayout(
