...
/**
 * `LAYOUT_FIT_BOX` (opcode [Operations.LAYOUT_FIT_BOX]) — a fit-box layout container.
 *
 * Wire layout: opcode byte + int `componentId` + int `animationId` + int `horizontalPositioning` +
 * int `verticalPositioning` (mirrors upstream `FitBoxLayout.apply`/`read`).
 */
class FitBoxLayout(
    val componentId: Int,
    val animationId: Int,
    ...
    override val opcode: Int get() = Operations.LAYOUT_FIT_BOX
    ...
    override fun dump(): String =
        "LAYOUT_FIT_BOX id=$componentId anim=$animationId hPos=$horizontalPositioning " +
            "vPos=$verticalPositioning"

    override fun equals(other: Any?): Boolean =
        this === other || (
            other is FitBoxLayout &&
