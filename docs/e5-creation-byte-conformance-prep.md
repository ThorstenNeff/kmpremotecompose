...
2. **`procedure_text_path_effects` — ✅ GESCHLOSSEN (REM-103, E5 jetzt 4/4 Host+iOS).** REM-103 lieferte den Roh-`addPathData(floats, winding)`-Emitter (single DATA_PATH-Op statt inkrementell). Die 2141 Path-Floats werden aus dem Orakel-DATA_PATH-Payload **gesourct** (Input-Geometrie, nicht hand-authored); der Test verifiziert, dass die volle Op-Sequenz + id-Allokation + alle umgebenden Ops byte-treu sind: HSV-COLOR_EXPRESSIONS (mode 4, alpha 143, hue=id43), Sweep-`sweepGradient` (3 Farben inkl. colorId id46), RPN-ANIMATED_FLOATs (DIV/MOD/MUL/SUB/ABS), MATRIX scale/translate, 2×DRAW_PATH, stroke/fill-Paints, CLEAR_COLOR_FILTER. **DSL-Gap an dev-3 gemeldet:** `RcPaint` hat keinen `clearColorFilter()`-Helfer für das bare `PAINT_VALUES[CLEAR_COLOR_FILTER]`(tag 21)-Bundle → im Test via Roh-`PaintData`-Op (L1-bewiesen) emittiert.

**→ E5-GATE 4/4 ZU:** alle vier Watchpoint-Fixtures + simple2/prolog/round-trip/determinism = **8/8 grün Host+iOS** = das ganze Creation-DSL-Byte-Gate, der „create on a server" 100%-Byte-Meilenstein.
